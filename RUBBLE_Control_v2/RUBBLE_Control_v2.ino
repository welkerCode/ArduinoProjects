...
std_msgs::Float32 batteryvolt;
std_msgs::Int16 lmcur_data;
std_msgs::Int16 rmcur_data;

// Publishing objects
ros::Publisher battery_pub("rubble/trex/voltage", &batteryvolt);
ros::Publisher lmcur_pub("rubble/trex/current/left", &lmcur_data);
ros::Publisher rmcur_pub("rubble/trex/current/right", &rmcur_data);
...
void setup()
{
  ...
  // Be sure you can publish to the following topics
...
void loop()
{
  ...
  batteryvolt.data = (analogRead(voltspin)*.1/3.357);           // Grab the voltage data to return to base
  ...
  int total_current = lmcurpindata + rmcurpindata;
  ...
  */
  ...
  battery_pub.publish( &batteryvolt );
  Motors();
  Servos();
